...
def client():
    """
    The API client, created the first time it is needed.

    Created lazily rather than at import time, so that importing this module
    does not require a key. That means `python -m agent.cli --help` works
    before you have set anything up, and a missing key produces a message
    you can act on rather than a traceback from inside the SDK.
    """
    global _client
    if _client is None:
